...
export default async function Home({
  ...
  const home = await getHomePage({ searchParams: { preview } });
  if (!home) notFound();
  ...
  return (
    <>
      <main
        ...
        className="flex flex-col gap-3 overflow-x-hidden bg-(--background-color) px-3 pt-(--h-header) pb-3 text-(--text-color) **:[path]:fill-(--color-logo)"
        style={
          {
      ...
      >
        <section
          className="relative h-(--h-section) max-h-[70vw]"
          style={
            {
              "--h-section": "calc(100svh - var(--h-header))"
            } as React.CSSProperties
          }
        >
